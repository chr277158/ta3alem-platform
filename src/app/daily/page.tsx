...
export default function DailyChallengePage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center"><div className="text-2xl animate-pulse">جاري التحميل...</div></div>;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 flex items-center justify-center p-6" dir="rtl">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-2xl font-bold mb-4">{alreadyCompleted ? 'أكملت التحدي اليوم!' : 'حدث خطأ'}</h2>
          <p className="text-gray-600 mb-6">{error}</p>
          <button onClick={() => router.push('/dashboard')} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white py-3 rounded-xl font-bold hover:from-purple-700 hover:to-pink-700 transition-all">
            🏠 العودة للرئيسية
          </button>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 flex items-center justify-center p-6" dir="rtl">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full text-center animate-bounce-in">
          <div className="text-6xl mb-4">{scoreRef.current === 3 ? '🏆' : scoreRef.current >= 2 ? '🎉' : '💪'}</div>
          <h2 className="text-3xl font-bold mb-4">{scoreRef.current === 3 ? 'ممتاز!' : scoreRef.current >= 2 ? 'أحسنت!' : 'حاول مرة أخرى'}</h2>
          <div className="text-5xl font-bold text-blue-600 mb-2">{scoreRef.current}/{questions.length}</div>
          <p className="text-gray-600 mb-6">{scoreRef.current === 3 ? 'لقد أكملت التحدي اليومي بنجاح!' : 'عد غداً لمحاولة جديدة'}</p>
          <button onClick={() => router.push('/dashboard')} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white py-3 rounded-xl font-bold hover:from-purple-700 hover:to-pink-700 transition-all">
            🏠 العودة للرئيسية
          </button>
        </div>
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-2xl shadow-lg p-4 mb-6">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="text-lg font-bold text-gray-600">السؤال {currentQuestion + 1} من {questions.length}</div>
